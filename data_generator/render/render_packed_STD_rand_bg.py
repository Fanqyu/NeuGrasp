...
sys.path.append('/path/to/NeuGrasp/data_generator/render')
from modify_material import set_modify_material, set_modify_raw_material, set_modify_table_material, \
    set_modify_floor_material, set_modify_arm_material
...
code_root = "/path/to/NeuGrasp/data_generator/render/renderer"  # NOTE
urdf_path = code_root + '/data/urdfs/packed/train'  # NOTE
...
obj_texture_image_root_path = "/path/to/NeuGrasp/data/assets/imagenet"  # NOTE
obj_texture_image_idxfile = "train_paths.txt"

origin_root_path = '/path/to/NeuGraspData/data/giga_hemisphere_train_0827/packed_full/6_M_rand/packed_%d-%d' % (
    830 * (SCENE_NUM // 830), 830 * (SCENE_NUM // 830) + 830)  # NOTE: used to generate background

output_root_path = ("/path/to/NeuGraspData/data/test/packed_full_bg/6_M_rand"
                    "/packed_%d-%d") % (
                       830 * (SCENE_NUM // 830), 830 * (SCENE_NUM // 830) + 830)  # NOTE

raw_urdfs_and_poses_dir_path = ("/path/to/NeuGraspData/data/GIGA/data_packed_train_raw"
                                "/mesh_pose_list")  # NOTE
...
def genCameraPosition(look_at):
    ...
    alpha_delta_delta = alpha_delta / (num_point_hor - 1)
    beta_delta = (beta_range[0] - beta_range[1]) / (num_point_hor - 1)
...
class BlenderRenderer(object):
    ...
    def loadImages(self, pattern_path, env_map_path, real_table_image_root_path, real_floor_image_root_path):
        ...
        for item in os.listdir(real_table_image_root_path):
            if item.split('.')[-1] == 'jpg':
                self.realtable_img_list.append(
                    bpy.data.images.load(filepath=os.path.join(real_table_image_root_path, item)))
        # load real floor images
        for item in os.listdir(real_floor_image_root_path):
            if item.split('.')[-1] == 'jpg':
                self.realfloor_img_list.append(
                    bpy.data.images.load(filepath=os.path.join(real_floor_image_root_path, item)))
        ...
        start = random.randint(0, 99900)  # self.obj_texture_img_list可能为空的原因是之前用的val set，只有50000张，starts超过范围了
        end = start + 100
        for item in lines[start:end]:
            item = item[:-1]  # 去掉"\n"
            self.obj_texture_img_list.append(
                bpy.data.images.load(filepath=os.path.join(obj_texture_image_root_path, "images", item)))
        # print('--------------------', self.obj_texture_img_list, '----------------------')
        ###
    ...
    def addMaskMaterial(self, num=20):
        ...
        for material_name in background_material_name_list:
            material_class = (bpy.data.materials.get(material_name) or bpy.data.materials.new(
                material_name))  # test if material exists, if it does not exist, create it:
        ...
        for i in range(num):
            ...
            if class_name.split('_')[0] == 'background' or class_name.split('_')[0] == 'table' or class_name.split('_')[
                0] == 'tableplane' or class_name.split('_')[0] == 'arm':
                node_2.inputs[0].default_value = (1, 1, 1, 1)
            else:
    ...
    def renderEngineSelect(self, engine_mode):
        ...
        if engine_mode == "CYCLES":
            ...
            ## Set the device_type
            bpy.context.scene.cycles.device = 'GPU'  # NOTE
    ...
    def addBackground(self, size, position, scale):
        ...
        for i in range(-2, 3, 1):
            for j in range(-2, 3, 1):
                ...
                bpy.ops.mesh.primitive_plane_add(size=size, enter_editmode=False, align='WORLD', location=position_i_j,
                                                 scale=scale)
        ...
        for i in range(-2, 3, 1):
            for j in [-2, 2]:
                position_i_j = (
                    i * size + position[0], j * size + position[1], position[2] - 0.25)  # - TABLE_CAD_MODEL_HEIGHT)
    ...
    def set_material_randomize_mode(self, class_material_pairs, mat_randomize_mode, instance,
        ...
        if mat_randomize_mode in ['mixed', 'diffuse', 'transparent', 'specular_tex', 'specular_texmix']:
            if material_type_in_mixed_mode == 'raw':
                ...
                set_modify_material(instance, material, self.obj_texture_img_list,
...
def generate_CAD_model_list(urdf_path_list):
    CAD_model_list = {}
    for urdf_path in urdf_path_list:
        instance_path = urdf_path
        class_name = 'other'

        file_type = os.path.splitext(urdf_path)
        if file_type == '.urdf':
...
def generate_material_type(obj_name, class_material_pairs, instance_material_except_pairs,
                           instance_material_include_pairs, material_class_instance_pairs, material_type):
...
# urdfs_and_poses_files_list = urdfs_and_poses_files_list[1033:]  # NOTE 
urdfs_and_poses_dict = \
    np.load(os.path.join(raw_urdfs_and_poses_dir_path, urdfs_and_poses_files_list[SCENE_NUM]), allow_pickle=True)['pc']
...
for i, path in enumerate(urdf_path_list):
    urdf_path_list[i] = os.path.join(urdf_path, os.path.basename(path))
# print(urdf_path_list)
# generate CAD model list
CAD_model_list = generate_CAD_model_list(urdf_path_list)  # TODO
# print(CAD_model_list)
...
path_scene = os.path.join(output_root_path,
                          urdfs_and_poses_files_list[SCENE_NUM][:-4])  ### "scene_"+str(SCENE_NUM).zfill(4))

origin_path_scene = os.path.join(origin_root_path,
                          urdfs_and_poses_files_list[SCENE_NUM][:-4])  ### "scene_"+str(SCENE_NUM).zfill(4))
...
cam_RT = np.load(os.path.join(origin_path_scene, "camera_pose.npy"))
trans_array = cam_RT[:, :-1, -1:]
orientation = scipy.spatial.transform.Rotation.from_matrix(cam_RT[:, :-1, :-1])
quat_array = orientation.as_quat()
quat_array = np.concatenate([quat_array[:, -1:], quat_array[:, :-1]], axis=-1)
trans_list = trans_array
quat_list = quat_array
...
#
#     material_type_in_mixed_mode = generate_material_type(instance_name, class_material_pairs,
#                                                          instance_material_except_pairs,
#                                                          instance_material_include_pairs, material_class_instance_pairs,
#                                                          my_material_randomize_mode)
#
...
#
#     # set material
#     renderer.set_material_randomize_mode(class_material_pairs, my_material_randomize_mode, obj,
#                                          material_type_in_mixed_mode)
#
#     # generate meta file
...
if render_mode_list['IR'] or render_mode_list['RGB']:
    ...
    arm_material_type_in_mixed_mode = generate_material_type(None, class_material_pairs, instance_material_except_pairs,
                                                             instance_material_include_pairs,
                                                             material_class_instance_pairs, material_type="diffuse")
        ...
        for obj in bpy.data.objects:
            if obj.type == "MESH" and obj.name.split('_')[0] == 'background':
                if obj.name == 'background_0':
                    # flag = random.randint(0, 3)  # TODO
                    flag = 1
                    if flag == 0:
                        set_modify_floor_material(obj, background_material_list[0],
                                                  selected_realfloor_img)  ### renderer.realfloor_img_list)
                    else:
                ...
                # flag = random.randint(0, 2)  # TODO
                flag = 1
                if flag == 0:
                    set_modify_table_material(obj, table_material_list[0],
