...
from src.neugrasp.utils.base_utils import compute_relative_transformation, compute_F
import numpy as np
import cv2
...
def draw_gripper(img, R, t, K, width, thickness=2, dist=None, pos=True):
    rotation_vec = cv2.Rodrigues(R)[0]
    img = img.astype(np.float32)
    dist = np.zeros(4, dtype=float) if dist is None else dist
    points = np.float32(
        [[0, -width / 2, 0], [0, -width / 2, 0.05], [0, width / 2, 0], [0, width / 2, 0.05], [0, 0, -0.025],
         [0, 0, 0]]).reshape(-1, 3)
    axis_points, _ = cv2.projectPoints(points, rotation_vec, t, K, dist)
    ...
    if pos:
        img = cv2.line(img, tuple(axis_points[0].ravel()), tuple(axis_points[2].ravel()), (255, 165, 0), thickness)
        img = cv2.line(img, tuple(axis_points[0].ravel()), tuple(axis_points[1].ravel()), (255, 165, 0), thickness)
        img = cv2.line(img, tuple(axis_points[2].ravel()), tuple(axis_points[3].ravel()), (255, 165, 0), thickness)
        img = cv2.line(img, tuple(axis_points[-2].ravel()), tuple(axis_points[-1].ravel()), (255, 165, 0), thickness)
    else:
        img = cv2.line(img, tuple(axis_points[0].ravel()), tuple(axis_points[2].ravel()), (173, 216, 230), thickness)
        img = cv2.line(img, tuple(axis_points[0].ravel()), tuple(axis_points[1].ravel()), (173, 216, 230), thickness)
        img = cv2.line(img, tuple(axis_points[2].ravel()), tuple(axis_points[3].ravel()), (173, 216, 230), thickness)
        img = cv2.line(img, tuple(axis_points[-2].ravel()), tuple(axis_points[-1].ravel()), (173, 216, 230), thickness)
    return img
