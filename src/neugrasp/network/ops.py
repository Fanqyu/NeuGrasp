...
def interpolate_feats(feats, points, h=None, w=None, padding_mode='zeros', align_corners=False, inter_mode='bilinear'):
    ...
    if h is None and w is None:
        h, w = ch, cw
    x_norm = points[:, :, 0] / (w - 1) * 2 - 1  # [-1, 1]
    y_norm = points[:, :, 1] / (h - 1) * 2 - 1
    points_norm = torch.stack([x_norm, y_norm], -1).unsqueeze(1)  # [srn,1,n,2]
    feats_inter = F.grid_sample(feats, points_norm, mode=inter_mode, padding_mode=padding_mode,
                                align_corners=align_corners).squeeze(2)  # srn,f,n
...
class ResUNetLight(nn.Module):
    def __init__(self, in_dim=3, layers=(2, 3, 6, 3), out_dim=32, inplanes=32):  # 3, [1, 2, 6, 4], 32, inplanes=16
        ...
        self.inplanes = inplanes  # 随着self._make_layer的planes改变
    ...
    def skipconnect(self, x1, x2):
        diffY = x2.size()[2] - x1.size()[2]  # 行
        diffX = x2.size()[3] - x1.size()[3]  # 列
