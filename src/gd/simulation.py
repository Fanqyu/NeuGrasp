...
class ClutterRemovalSim(object):
    def __init__(self, scene, object_set, gui=True, seed=None, renderer_root_dir="", args=None):
        ...
        self.gripper = Gripper(self.world)  # TODO
        # self.gripper = None
        self.size = 0.3  # TODO
        self.table_height = 0.05  # TODO
    ...
    def reset(self, object_count, n_round):
        ...
        if self.args.gen_scene_descriptor:
            ...
            if self.scene == "pile":
                ...
                urdfs_and_poses_dict = self.generate_packedsingle_scene(object_count, self.table_height,
    ...
    def generate_pile_scene(self, object_count, table_height, scene_descriptor_npz=None):
        ...
        #       '/path/to/NeuGrasp/data/assets/data/mesh_pose_list/pile_pile_test_200/0000.npy'
        urdfs_and_poses_dict = {}
        if self.args.gen_scene_descriptor:
        ...
        #       '/path/to/NeuGraspData/data/GIGA/data_pile_train_raw/mesh_pose_list/
    ...
    def generate_packedsingle_scene(self, object_count, table_height, scene_descriptor_npz=None):  # TODO
        attempts = 0
        ...
        for _ in range(1):
            ...
            body = self.world.load_urdf(urdf, pose, scale=self.global_scaling * scale)
            lower, upper = self.world.p.getAABB(body.uid)
            z = table_height + 0.5 * (upper[2] - lower[2]) + 0.02
            body.set_pose(pose=Transform(rotation, np.r_[x, y, z]))
    ...
    def acquire_tsdf(self, n, N=None):
        ...
        origin = Transform(Rotation.identity(), np.r_[self.size / 2, self.size / 2, 0.0503])
        r = 2.0 * self.size

        N = N if N else n
        phi_list = 2.0 * np.pi * np.arange(n) / N
        theta_list = np.pi * np.arange(1, 4) / 8

        # my view
        # phi_list = 4 * np.pi / 3 + np.arange(0, 4) * np.pi * 20 / 180
        # theta_list = np.pi / 4 - np.arange(0, 4) * np.pi / 18
...
class Gripper(object):
    ...
    def __init__(self, world):
        self.world = world
        # self.urdf_path = Path("data/assets/data/urdfs/panda/hand.urdf")  # NOTE put in cfg
        self.urdf_path = Path("/path/to/NeuGrasp/data/assets/data/urdfs/panda/hand.urdf")  # NOTE put in cfg
        self.max_opening_width = 0.08
        self.finger_depth = 0.05  # used to sample
...
#     w = btsim.BtWorld()
#     g = Gripper(w)
#     T_world_tcp = Transform(Rotation.identity(), [0., 0., 0.])
#     g.reset(T_world_tcp)
#     time.sleep(3)
#     T_world_grasp = Transform(Rotation.identity(), [0.8, 0.8, 0.1])
#     g.move_tcp_xyz(T_world_grasp)
#     time.sleep(3)
#     g.move(0.)
#     print(f'[I] Open scale: {g.read()}')
#     time.sleep(3)
#     g.move(1.)
#     print(f'[I] Open scale: {g.read()}')
#     time.sleep(3)
#     T_grasp_retreat = Transform(Rotation.identity(), [0.0, 0.0, -0.1])
#     T_world_retreat = T_world_grasp * T_grasp_retreat
#     g.move_tcp_xyz(T_world_retreat)
#     time.sleep(30)
