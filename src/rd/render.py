...
from src.rd.modify_material import set_modify_table_material, set_modify_floor_material
from src.rd.render_utils import *


def blender_init_scene(code_root, log_root_dir, obj_texture_image_root_path, scene_type, urdfs_and_poses_dict,
                       round_idx, logdir, check_seen_scene, material_type, gpuid, output_modality_dict):
    if scene_type == "pile":
    ...
    urdf_path_list = [os.path.join(value[3]) for value in
                      urdfs_and_poses_dict.values()]  # "/".join(code_root.split("/")[:-2]),
    ...
    renderer.loadImages(emitter_pattern_path, env_map_path, real_table_image_root_path, real_floor_image_root_path,
                        obj_texture_image_root_path, obj_texture_image_idxfile, check_seen_scene)
    renderer.addEnvMap()
    ...
    path_scene = output_root_path  # os.path.join(output_root_path, uuid.uuid4().hex, "init") ### "scene_"+str(
    # SCENE_NUM).zfill(4))
    ...
    if my_material_randomize_mode == 'raw':
        ...
        material_selected = random.sample(renderer.my_material['background'], 1)[
            0]  ### renderer.my_material['background'][1]
    ...
    for model in select_model_list:
        ...
        instance_name = str(
            instance_id) + "_" + class_name + "_" + instance_folder + "_" + instance_uid  ### class_folder + "_" + instance_folder
        ...
        material_type_in_mixed_mode = generate_material_type(instance_name, class_material_pairs,
                                                             ...
                                                             material_class_instance_pairs, material_type)
        ...
        # generate meta file
        class_scale = obj_scale_list[
            instance_id - 1]  ### random.uniform(g_synset_name_scale_pairs[class_name][0], g_synset_name_scale_pairs[class_name][1])
        obj.scale = (class_scale, class_scale, class_scale)
...
def blender_update_sceneobj(obj_name_list, obj_trans_list, obj_quat_list, obj_uid_list):
    for obj_name in bpy.data.objects.keys():
        ...
        if obj.type == 'MESH' and obj_name[0:10] != "background" and obj_name not in ['camera_l', 'camera_r',
                                                                                      'light_emitter', 'table',
                                                                                      'tableplane']:
            obj_uid = obj_name.split("_")[-1]
            if obj_uid not in obj_uid_list:
                print("[V]blender_update_sceneobj: obj_uid [not in] obj_uid_list: ", obj_name, obj_name_list, obj_uid,
                      obj_uid_list)
                ...
                print("[V]blender_update_sceneobj: obj_uid [in] obj_uid_list: ", obj_name, obj_name_list, obj_uid,
                      obj_uid_list, obj_world_pose_T, obj_world_pose_quat)
                setModelPosition(obj, obj_world_pose_T, obj_world_pose_quat)


def blender_render(renderer, quaternion_list, translation_list, path_scene, render_frame_list, output_modality_dict,
                   camera_focal, is_init=False):
    # set the key frame
    ...
    if output_modality_dict['IR'] or output_modality_dict['RGB']:
        if is_init:
            renderer.src_energy_for_rgb_render = bpy.data.worlds["World"].node_tree.nodes["Background"].inputs[
                1].default_value
        ...
        for i in render_frame_list:
            ...
            if output_modality_dict['IR']:
                ...
                if not os.path.exists(ir_l_dir_path):
                    os.makedirs(ir_l_dir_path)
                ir_r_dir_path = os.path.join(path_scene, 'ir_r')
                if not os.path.exists(ir_r_dir_path):
