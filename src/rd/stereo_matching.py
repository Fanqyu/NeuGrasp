                        ...
                        for r in range(-1, 2, 1):
                            for c in range(-1, 2, 1):
                                ...
                                if rowr >= 0 and rowr < H and colc >= 0 and colc < W:
                                    if mask_visited[rowr, colc] == 0 and torch.abs(
                                            disparity_map[b, :, rowr, colc] - disp_base) <= diff_insame:
...
class StereoMatching(nn.Module):
    ...
    def forward(self, imageL, imageR, f, blDis, beta, sigmaColor=0.05, sigmaSpace=5):
        ...
        for i in range(self.minDisp, D + 1, 1):
            ...
            costVolumeL[i - self.minDisp] = CorrL(i, cacheImageL, cacheImageR, filters, padding, self.blockSize,
                                                  self.eps)
            costVolumeR[i - self.minDisp] = CorrR(i, cacheImageL, cacheImageR, filters, padding, self.blockSize,
                                                  self.eps)
...
class DREDS(Dataset):
    def __init__(self, root, scale):
        ...
        for img_id in sorted(os.listdir(self.root + "/ir_l")):
            img_id = os.path.join(self.root, 'ir_l', img_id)
            if not os.path.exists(img_id[:-4] + "_simDepthImage.exr"):
                path = img_id[:-4]
                self.ir_l_list.append(img_id)
                self.depth_list.append(path + "_depth_120.exr")
        for img_id in sorted(os.listdir(self.root + "/ir_r")):
            img_id = os.path.join(self.root, 'ir_r', img_id)
            self.ir_r_list.append(img_id)
    ...
    def __getitem__(self, index):
        ...
        imageL = np.array(imageL.resize((int(imageL.size[0] * self.scale), int(imageL.size[1] * self.scale)))).astype(
            np.float32)
        imageR = np.array(imageR.resize((int(imageR.size[0] * self.scale), int(imageR.size[1] * self.scale)))).astype(
            np.float32)
...
def main_batch(root="/path/to/temp/test_IR"):
    ###########################
    # Parameter setting
    ###########################
    scale = 1  # 0.5  # image scale
    beta = torch.tensor(100.)  # 100 # beta para of softArgmax
    blockSize = 11  # 11 # stereo matching para
    maxDisp = 110  # max disparity between left and right images
    minDisp = 1  # min disparity between left and right images
    f = torch.tensor(459.14)  # focal length of the sensor
    ...
    for i, (imageLTensor, imageRTensor, ir_l_path, ir_r_path, depthTensor) in enumerate(DataLoader):
        ...
        # stereo matching
        depthImage, pointCloud = object(imageLTensor, imageRTensor, f, baselineDis, beta, sigmaColor,
                                        sigmaSpace)  # [B C H W]
        ...
        for i in range(imageLTensor.shape[0]):
            ...
            save_dir = os.path.join(save_root, 'sim_depth')
            ...
            print(save_dir + "/%s.exr" % ir_l_path[i].split("/")[-1][:4])
            cv2.imwrite(save_dir + "/%s.exr" % ir_l_path[i].split("/")[-1][:4], depthImageExr)
